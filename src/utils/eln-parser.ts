...
export class ELNParser {
  ...
  private extractDatasets(crateData: ROCrateData | null | undefined): ELabFTWDataset[] {
    ...
    for (const item of crateData['@graph']) {
      // P0 FIX: Don't require genre field - use fallback to prevent data loss
      if (item['@type'] === 'Dataset') {
        ...
        const dataset: ELabFTWDataset = {
          ...
          mentions: this.extractMentions(item.mentions),  // BUG FIX: handles null/undefined and objects
          files: this.extractFiles(item.hasPart),  // BUG FIX: handles null/undefined and objects
  ...
  /**
   * BUG FIX: Handle mentions as either array or single object
   * SampleDB exports use single object: "mentions": { "@id": "..." }
   * eLabFTW exports use array: "mentions": [{ "@id": "..." }]
   */
  ...
  /**
   * BUG FIX: Handle hasPart as either array or single object
   * Some ELN exports may use single object instead of array
   */
  private extractFiles(hasPart: any[] | any): string[] {
    // Handle null/undefined
    if (!hasPart) return [];

    // Normalize to array
    const hasPartArray = Array.isArray(hasPart) ? hasPart : [hasPart];
